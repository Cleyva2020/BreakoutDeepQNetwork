...
#         self.build_network()
#         self.sess.run(tf.global_variables_iniatializer())
...
#             self.actions = tf.placeholder(tf.float32, shape=[None, self.input_dims], name='action_taken')
#             self.q_target = tf.placeholder(tf.float32, shape=[None, self.input_dims], name='q_value')
#
...
#
#             conv2 = tf.layers.conv2d(inputs=convl_activated, filters=64, kernel_size=(4,4), strides=2, name='conv2', kernel_initializer=tf.variance_scaling_initializer(scale=2))
...
#             densel = tf.layers.dense(flat, units=self.fcl_dims, activation=tf.nn.relu, kernel_initializer=tf.variance_scaling_initializer(scale=2))
#
...
#
#     class Agent(object):
#         def __init__(self, alpha, gamma, gamma, mem_size, n_actions, epsilon, batch_size, replace_target=5000, input_dims=(210,160,4), q_next='tmp/q_next', q_eval='tmp/q_eval'):
#             self.n_actions = n_actions
...
#             actions[action] = 1.0
#             self.action_reward[index] = actions
...
#             new_state_batch = self.new_state_batch[batch]
#             terminal_batch = self.terminal_memory[batch]
#
#             q_eval = self.q_eval.sess.run(self.q_eval.Q_values, feed_dict={self.q_eval.input: state_batch})
#             q_next = self.q_next.sess.run(self.q_eval.Q_values, feed_dict={self.q_eval.input: new_state_batch})
#
